...
- **No separate merge-gating automation for PR comments**: do not add local or CI layers on top of GitHub review blocking. The requirement is to inspect inline review comments, top-level PR conversation comments, and latest review summaries before merge, then reply/resolve everything that is still actionable. See `.claude/rules/pr-comment-handling.md`.
