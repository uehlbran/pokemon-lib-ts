...
- **Manual PR comment audit is mandatory**: do not trust local scripts or CI to decide whether comments are fully handled. Before merge, inspect inline review comments, top-level PR conversation comments, and latest review summaries. The reminder hook exists only to force that checklist back into view. See `.claude/rules/pr-comment-handling.md`.
