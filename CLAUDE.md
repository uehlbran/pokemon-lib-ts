...
- **Track review comments until merge** — mandatory. Every inline review comment must get a reply, every review thread must be resolved before merging, and actionable top-level PR comments plus latest review-summary findings must have a later author acknowledgment before merge.
- **No separate comment gate**: do not add local or CI comment-gate automation. GitHub review state already blocks unresolved review threads. The requirement is to inspect inline review comments, top-level PR conversation comments, and latest review summaries before merge, then reply/resolve everything that is still actionable. See `.claude/rules/pr-comment-handling.md`.
...
- **CodeRabbit and Qodo are advisory on substance, not optional on handling** — validate each report against the current code, then fix it, reply with rationale, or file a follow-up issue if it is real and out of scope. Do not rely on them as verification; required local verification is `npm run verify:local`.
