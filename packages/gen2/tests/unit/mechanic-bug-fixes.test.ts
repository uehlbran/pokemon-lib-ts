...
import type { PokemonInstance, PokemonType, PrimaryStatus, TypeChart } from "@pokemon-lib-ts/core";
import {
  CORE_ABILITY_IDS,
...
function createMoveSlotFixture(moveId: string = MOVE_IDS.tackle, pp?: number, maxPp?: number) {
  const baseMove = dataManager.getMove(moveId);
  const resolvedMaxPp = maxPp ?? baseMove.pp;
  return createMoveSlot(baseMove.id, pp ?? resolvedMaxPp, 0);
}
...
function createOnFieldPokemonFixture(
  ...
  return {
    pokemon: {
      ...
      ivs: createDvs(),
      evs: createEvs(),
      moves: overrides.moves ?? [createMoveSlotFixture()],
      calculatedStats: {
        hp: maxHp,
...
function createFixedRng(intReturnValue: number) {
  return {
    next: () => 0,
...
describe("#253 — Hyper Beam recharge skipped only on KO, NOT on miss", () => {
  ...
  it("given Hyper Beam KOs the target (currentHp === 0), when executeMoveEffect, then noRecharge is true", () => {
    ...
    const attacker = createOnFieldPokemonFixture();
    const defender = createOnFieldPokemonFixture({ currentHp: 0, maxHp: 200 });
    const state = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
    );
  ...
  it("given Hyper Beam misses (damage=0, defender alive), when executeMoveEffect, then noRecharge is NOT set", () => {
    ...
    const attacker = createOnFieldPokemonFixture();
    const defender = createOnFieldPokemonFixture({ currentHp: 150, maxHp: 200 });
    const state = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
    );
  ...
  it("given Hyper Beam hits but target survives, when executeMoveEffect, then noRecharge is NOT set", () => {
    ...
    const attacker = createOnFieldPokemonFixture();
    const defender = createOnFieldPokemonFixture({ currentHp: 50, maxHp: 200 });
    const state = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
    );
...
describe("#325 — Attract and Nightmare volatile cleared on switch-out", () => {
  it("given Pokemon has Attract volatile, when switching out, then Attract is cleared", () => {
    ...
    const pokemon = createOnFieldPokemonFixture({ lastMoveUsed: MOVE_IDS.tackle });
...
describe("#328 — Baton Pass should transfer perish-song, substitute, and curse", () => {
  it("given Baton Pass switch, when onSwitchOut is called, then perish-song is preserved", () => {
    ...
    const pokemon = createOnFieldPokemonFixture({ lastMoveUsed: MOVE_IDS.batonPass });
...
describe("#329 — Bright Powder reduces opponent's accuracy by 20/256", () => {
  it("given defender holds Bright Powder, when 100% accurate move used, then accuracy is reduced and can miss", () => {
    ...
    const attacker = createOnFieldPokemonFixture();
    const defender = createOnFieldPokemonFixture({ heldItem: ITEM_IDS.brightPowder });
    ...
    const rngMiss = createFixedRng(240);
  ...
  it("given defender does NOT hold Bright Powder, when 100% accurate move and RNG=240, then move hits", () => {
    ...
    const attacker = createOnFieldPokemonFixture();
    const defender = createOnFieldPokemonFixture({ heldItem: null });
    const move = dataManager.getMove(MOVE_IDS.bodySlam);
    const state = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
...
describe("#330 — SolarBeam halved in rain/sandstorm; Thunder always-hit in rain", () => {
  ...
  it("given sun weather, when Thunder with 70% accuracy used and RNG=100, then it hits at halved accuracy", () => {
    ...
    const defender = createOnFieldPokemonFixture();
    const move = dataManager.getMove(MOVE_IDS.thunder);
    const stateSun = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
      {
  ...
  it("given rain weather, when SolarBeam is used, then power is halved", () => {
    ...
    const stateRain = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
      {
        type: WEATHER_IDS.rain,
    ...
    const stateNone = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
      null,
    );
  ...
  it("given sandstorm weather, when SolarBeam is used, then power is halved", () => {
    ...
    const stateSand = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
      {
...
describe("#331 — Weather-dependent healing for Moonlight/Morning Sun/Synthesis", () => {
  it("given no weather, when Moonlight is used, then heals 1/2 max HP", () => {
    ...
    const attacker = createOnFieldPokemonFixture({ maxHp: 200, currentHp: 50 });
    const defender = createOnFieldPokemonFixture();
    const state = createBattleStateFixture(
      createBattleSideFixture(0, attacker),
      createBattleSideFixture(1, defender),
