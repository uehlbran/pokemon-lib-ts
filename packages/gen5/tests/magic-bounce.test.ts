...
import { createGen5DataManager } from "../src";
import {
...
function getMagicBounceMessage(defender: ActivePokemon, move: MoveData): string {
  ...
  const magicBounceName = dataManager.getAbility(TEST_ABILITIES.magicBounce).displayName;
...
describe("shouldReflectMoveGen5", () => {
  it("given defender with magic-bounce, when Spore targets it, then move is reflected", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce, nickname: "Espeon" });
    const move = makeStatusMove(TEST_MOVE_IDS.spore, "Spore");
  ...
  it("given defender with magic-bounce, when Thunder Wave targets it, then move is reflected", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce, nickname: "Natu" });
    const move = makeStatusMove(TEST_MOVE_IDS.thunderWave, "Thunder Wave");
    const state = makeBattleState();
  ...
  it("given defender with magic-bounce, when Flamethrower targets it, then move is NOT reflected (damaging)", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeDamagingMove(TEST_MOVE_IDS.flamethrower, "Flamethrower");
    const state = makeBattleState();
    ...
    expect(defender.ability).toBe(TEST_ABILITIES.magicBounce);
  });
  ...
  it("given defender WITHOUT magic-bounce, when Spore targets it, then move is NOT reflected", () => {
    ...
    const attacker = makeActivePokemon({ ability: "overgrow" });
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.pressure });
    const move = makeStatusMove(TEST_MOVE_IDS.spore, "Spore");
    const state = makeBattleState();
    ...
    expect(defender.ability).toBe(TEST_ABILITIES.pressure);
  });
  ...
  it("given defender with magic-bounce and attacker with mold-breaker, when Spore targets it, then move is NOT reflected", () => {
    ...
    const attacker = makeActivePokemon({ ability: TEST_ABILITIES.moldBreaker });
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeStatusMove(TEST_MOVE_IDS.spore, "Spore");
    const state = makeBattleState();
    ...
    expect(result).toBeNull();
    expect(attacker.ability).toBe(TEST_ABILITIES.moldBreaker);
    expect(defender.ability).toBe(TEST_ABILITIES.magicBounce);
  });
  ...
  it("given defender with magic-bounce and attacker with teravolt, when Toxic targets it, then move is NOT reflected", () => {
    ...
    const attacker = makeActivePokemon({ ability: TEST_ABILITIES.teravolt });
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeStatusMove(TEST_MOVE_IDS.toxic, "Toxic");
    const state = makeBattleState();

    const result = shouldReflectMoveGen5(move, attacker, defender, state);

    expect(result).toBeNull();
    expect(attacker.ability).toBe(TEST_ABILITIES.teravolt);
    expect(defender.ability).toBe(TEST_ABILITIES.magicBounce);
  });
  ...
  it("given defender with magic-bounce and attacker with turboblaze, when Will-O-Wisp targets it, then move is NOT reflected", () => {
    ...
    const attacker = makeActivePokemon({ ability: TEST_ABILITIES.turboblaze });
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeStatusMove(TEST_MOVE_IDS.willOWisp, "Will-O-Wisp");
    const state = makeBattleState();

    const result = shouldReflectMoveGen5(move, attacker, defender, state);

    expect(result).toBeNull();
    expect(attacker.ability).toBe(TEST_ABILITIES.turboblaze);
    expect(defender.ability).toBe(TEST_ABILITIES.magicBounce);
  });
  ...
  it("given defender with magic-bounce who is semi-invulnerable (flying), when Taunt targets it, then move is NOT reflected", () => {
    ...
    flyingVolatiles.set(TEST_VOLATILES.flying, { turnsLeft: 1 });
    const defender = makeActivePokemon({
      ability: TEST_ABILITIES.magicBounce,
      volatiles: flyingVolatiles,
    });
    const move = makeStatusMove(TEST_MOVE_IDS.taunt, "Taunt");
    const state = makeBattleState();

    const result = shouldReflectMoveGen5(move, attacker, defender, state);

    expect(result).toBeNull();
    expect(defender.volatileStatuses.has(TEST_VOLATILES.flying)).toBe(true);
    expect(defender.ability).toBe(TEST_ABILITIES.magicBounce);
  });
  ...
  it("given defender with magic-bounce, when a non-reflectable status move (Trick) targets it, then move is NOT reflected", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeStatusMove(TEST_MOVE_IDS.trick, "Trick");
    const state = makeBattleState();
  ...
  it("given defender with magic-bounce, when Stealth Rock targets it, then move is reflected (entry hazard)", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce, nickname: "Xatu" });
    const move = makeStatusMove(TEST_MOVE_IDS.stealthRock, "Stealth Rock");
    const state = makeBattleState();
  ...
  it("given defender with magic-bounce, when Roar targets it, then move is reflected", () => {
    ...
    const defender = makeActivePokemon({ ability: TEST_ABILITIES.magicBounce });
    const move = makeStatusMove(TEST_MOVE_IDS.roar, "Roar");
    const state = makeBattleState();
  ...
  it("given defender with magic-bounce but no nickname, reflection message uses speciesId", () => {
    ...
    const move = makeStatusMove(TEST_MOVE_IDS.encore, "Encore");
    const state = makeBattleState();
