...
const DEFAULT_SPECIES_ID = GEN6_DATA.getSpecies(SPECIES.pikachu).id;
const DEFAULT_NATURE = GEN6_DATA.getNature(NATURES.hardy).id;
...
function makeMove(moveId: string): MoveData {
  const base = GEN6_DATA.getMove(moveId);
...
describe("Gen6 Cloud Nine weather chip suppression", () => {
  ...
  it("given no suppression in hail, when applying weather effects, then chip damage is dealt to non-immune Pokemon", () => {
    ...
    const normalMon2 = makeActive({ ability: ABILITIES.blaze, types: [TYPES.fire], hp: 240, currentHp: 240 });
    const state = makeState({
      weather: { type: WEATHERS.hail, turnsLeft: 5, source: "test" },
