...
describe("Gen 9 DataManager -- data loading", () => {
  ...
  it("given gen9 data files, when loading DataManager, then loads at least 700 Pokemon", () => {
    // Source: Data generated by tools/data-importer/src/import-gen.ts --gen=9
    // Gen 9 (Scarlet/Violet) has a regional dex -- some older Pokemon are not
    // available. The importer pulls species #1-#1025 but skips those not in
    // Showdown's Gen 9 data. Expect at least 700 species.
    const dm = createGen9DataManager();
    expect(dm.getAllSpecies().length).toBeGreaterThanOrEqual(700);
  });
