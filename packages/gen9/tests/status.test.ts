/**
 * Gen 9 status damage and speed modifier tests.
 *
 * Gen 9 carries forward the Gen 7-8 changes:
 ...
 *   - Confusion self-hit: 33% (Gen 7+ override, was 50% in Gen 3-6)
 *
 * Source: Showdown sim/battle-actions.ts -- Gen 9 status damage unchanged from Gen 7-8
...
const DATA = createGen9DataManager();
const DEFAULT_SPECIES_ID = DATA.getSpecies(SPECIES.pikachu).id;
const DEFAULT_NATURE = DATA.getNature(NATURES.hardy).id;
...
const ruleset = new Gen9Ruleset(DATA);

// ===========================================================================
// Burn damage -- 1/16 max HP in Gen 9 (same as Gen 7-8)
// ===========================================================================

describe("Gen9Ruleset -- applyStatusDamage (burn)", () => {
...
describe("Gen9Ruleset -- applyStatusDamage (poison)", () => {
...
describe("Gen9Ruleset -- applyStatusDamage (badly-poisoned)", () => {
...
describe("Gen9Ruleset -- paralysis speed via resolveTurnOrder", () => {
  /**
...
// ===========================================================================
// Confusion self-hit -- 33% (Gen 7+ override)
// ===========================================================================

describe("Gen9Ruleset -- confusion self-hit", () => {
  ...
  it("given getConfusionSelfHitChance(), then it is NOT the Gen 3-6 value of 0.5", () => {
    // Source: Bulbapedia -- Gen 7+ changed confusion from 50% to 33%
    // Triangulation: verify it's specifically 1/3, not the old 50%
    expect(ruleset.getConfusionSelfHitChance()).not.toBeCloseTo(0.5);
  });

  it("given SeededRandom that always returns true for chance(), when rollConfusionSelfHit, then returns true", () => {
    // Source: Showdown data/conditions.ts -- Gen 7+ confusion uses rng.chance(1/3)
    const rng = {
      next: () => 0.1,
      int: (min: number, _max: number) => min,
      chance: () => true,
  ...
  it("given SeededRandom that always returns false for chance(), when rollConfusionSelfHit, then returns false", () => {
    // Source: Showdown data/conditions.ts -- Gen 7+ confusion uses rng.chance(1/3)
    const rng = {
      next: () => 0.9,
      int: (min: number, _max: number) => min,
