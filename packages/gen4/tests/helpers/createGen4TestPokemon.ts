...
import { createGen4DataManager, GEN4_MOVE_IDS, GEN4_NATURE_IDS, GEN4_SPECIES_IDS } from "../../src";

const dataManager = createGen4DataManager();
const defaultSpecies = dataManager.getSpecies(GEN4_SPECIES_IDS.bulbasaur);
const defaultMove = dataManager.getMove(GEN4_MOVE_IDS.tackle);

export interface CreateGen4TestPokemonOptions {
...
export function createGen4TestPokemon(options: CreateGen4TestPokemonOptions = {}): PokemonInstance {
  ...
  const pokemon = createPokemonInstance(
    ...
    {
      nature: options.nature ?? GEN4_NATURE_IDS.hardy,
