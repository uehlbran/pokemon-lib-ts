...
//   - Crit rate table: [24, 8, 2, 1] (stages 0-3+, same as Gen 6)
...
describe("Gen 7 critical hit constants", () => {
  it("given GEN7_CRIT_RATE_TABLE, then stage 0 denominator is 24 (~4.2%)", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- stage 0: 1/24
    expect(GEN7_CRIT_RATE_TABLE[0]).toBe(24);
  });

  it("given GEN7_CRIT_RATE_TABLE, then stage 1 denominator is 8 (12.5%)", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- stage 1: 1/8
    expect(GEN7_CRIT_RATE_TABLE[1]).toBe(8);
  });

  it("given GEN7_CRIT_RATE_TABLE, then stage 2 denominator is 2 (50%)", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- stage 2: 1/2
    expect(GEN7_CRIT_RATE_TABLE[2]).toBe(2);
  });

  it("given GEN7_CRIT_RATE_TABLE, then stage 3+ denominator is 1 (guaranteed)", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- stage 3+: guaranteed (1/1)
    expect(GEN7_CRIT_RATE_TABLE[3]).toBe(1);
  });

  it("given GEN7_CRIT_RATE_TABLE, then it has exactly 4 entries", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- 4 stages (0, 1, 2, 3+)
    expect(GEN7_CRIT_RATE_TABLE).toHaveLength(4);
  });
  ...
  it("given GEN7_CRIT_MULTIPLIER, then it is 1.5", () => {
    // Source: Bulbapedia "Critical hit" Gen 7 -- multiplier remains 1.5x (same as Gen 6)
    // Source: Showdown sim/battle-actions.ts -- Gen 6+ crit multiplier
    expect(GEN7_CRIT_MULTIPLIER).toBe(1.5);
  });

  it("given GEN7_CRIT_MULTIPLIER is 1.5, then it differs from Gen 5's 2.0x", () => {
    ...
    expect(GEN7_CRIT_MULTIPLIER).not.toBe(2.0);
    expect(GEN7_CRIT_MULTIPLIER).toBe(1.5);
  });
});

// ---------------------------------------------------------------------------
// Gen 7 Critical Hit Roll (via Gen7Ruleset.rollCritical)
//
// Gen 7 inherits the BaseRuleset.rollCritical logic but adds Battle Armor
// and Shell Armor immunity (same as Gen 6).
//
...
describe("Gen 7 critical hit roll behavior", () => {
